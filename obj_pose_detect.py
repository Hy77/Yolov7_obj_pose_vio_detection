...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        orig_size = im0s.shape[1], im0s.shape[0]  # record ori img size

        # Resize image for pose estimation
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if not dataset.mode == 'image':  # 如果是视频流
                        if vid_writer is None or vid_path != path:  # 如果需要初始化新的视频写入器
                            vid_path = path

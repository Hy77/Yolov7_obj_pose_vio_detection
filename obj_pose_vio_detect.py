...
rf = Roboflow(api_key="2SNgdleBjtHCtGoA3rjU")  # other API_Key: YPOJa0buvSk43vfGiCt2
...
def run_vio_detection(im0s):
    roboflow_prediction = roboflow_model.predict(im0s, confidence=40, overlap=30).json()

    # Filter predictions for 'violence' class only
    violence_predictions = [pred for pred in roboflow_prediction["predictions"] if pred["class"] == "Violence"]

    # Check if any violence is detected
    violence_detected = bool(violence_predictions)

    if violence_detected:
        # Update the predictions in the Roboflow result with only violence detections
        filtered_prediction = roboflow_prediction.copy()
        filtered_prediction["predictions"] = violence_predictions

        # Parse and draw bounding boxes for violence detections
        labels = [pred["class"] for pred in violence_predictions]
        detections = Detections.from_roboflow(filtered_prediction)
        box_annotator = BoxAnnotator()
        label_annotator = LabelAnnotator()

        # Draw bounding boxes and labels for 'violence' class only
        im0s = box_annotator.annotate(scene=im0s, detections=detections)
        im0s = label_annotator.annotate(scene=im0s, detections=detections, labels=labels)
    return im0s
